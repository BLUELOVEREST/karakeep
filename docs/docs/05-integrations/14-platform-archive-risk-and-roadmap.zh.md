...
后续 B 站视频归档建议补齐弹幕文件保存能力。

目标语义：

- B 站视频链接仍按视频站策略处理，只保留封面、视频和 metadata，不生成完整网页归档。
- 在视频下载成功时同步保存弹幕。
- 弹幕优先保存原始 `.danmaku.xml`，后续可选生成播放器更容易加载的 `.ass`。
- 弹幕文件作为 bookmark 的附加 asset 保存，便于前端后续展示或下载。

推荐实现路线：

- 第一阶段优先复用当前 Docker 镜像内已有的 `yt-dlp`。
- 使用 `yt-dlp --write-subs` 验证 B 站 `.danmaku.xml` 输出能力。
- 如果需要 `.ass`，再评估 `yt-dlp-danmaku` 插件，不优先引入完整 B 站专用下载服务。
- 如果后续遇到登录态、会员清晰度、合集或弹幕获取限制，再评估接入 `yutto` 这类 B 站专用工具。

需要关注：

- 弹幕文件与视频 asset 的关联方式。
- 是否需要新增 asset type，还是先按通用附件保存。
- B 站高码率或会员内容可能需要 cookies。
- 弹幕 XML 和 ASS 的编码、文件名、前端下载展示方式。
